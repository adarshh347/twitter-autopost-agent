...
try:
    ...
    from src.data_models import ScrapedTweet, ScrapedProfile
    from src.utils.scroller import Scroller
    from src.utils.progress import Progress

from .parsing import parse_tweet_card, parse_profile_page
...
class TweetScraper:
    ...
    def scrape_profile(self, profile_url: str) -> Optional[ScrapedProfile]:
        ...
        Args:
            profile_url: URL of the profile to scrape (e.g., "https://x.com/Tony_dsgn")
            
        Returns:
            ScrapedProfile object with user details, or None if scraping fails
        """
        logger.info("Navigating to profile: %s for profile details extraction", profile_url)
        
        # Ensure URL format is correct
        if not profile_url.startswith("http"):
            profile_url = f"https://x.com/{profile_url.lstrip('@')}"
        
        self.browser_manager.navigate_to(profile_url)
        time.sleep(5)  # Wait for page to fully load
        
        # Parse the profile page
        profile_data = parse_profile_page(self.driver, profile_url, logger)
        
        if profile_data:
            logger.info("Successfully scraped profile: %s (%s)", 
                       profile_data.user_name, profile_data.user_handle)
        else:
            logger.warning("Failed to extract profile data from: %s", profile_url)
