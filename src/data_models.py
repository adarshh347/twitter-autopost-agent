...
class ScrapedProfile(BaseModel):
    """Data model for storing scraped Twitter/X profile information."""
    user_id: Optional[str] = None  # Unique user ID if available
    user_name: str  # Display name
    user_handle: str  # @username
    bio: Optional[str] = None  # Profile description/bio
    location: Optional[str] = None  # User's location
    website: Optional[str] = None  # User's website URL
    joined_date: Optional[str] = None  # When the user joined Twitter/X
    birth_date: Optional[str] = None  # User's birthdate if public
    
    # Metrics
    followers_count: int = 0
    following_count: int = 0
    tweets_count: Optional[int] = None  # Total tweets/posts
    
    # Verification and status
    is_verified: bool = False
    is_business: Optional[bool] = None
    is_protected: bool = False  # Private/protected account
