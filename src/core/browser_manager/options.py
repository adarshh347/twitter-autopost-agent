...
def configure_driver_options(
    ...
    *,
    ...
    # Set page load strategy for faster loads ('eager' = DOM ready, 'none' = don't wait)
    if page_load_strategy and page_load_strategy in ('eager', 'none', 'normal'):
        options.page_load_strategy = page_load_strategy
        logger.info(f"Page load strategy set to: {page_load_strategy}")
    ...
    # Reduce automation fingerprints for Chrome (skip when using undetected-chromedriver)
    if browser_type == 'chrome' and not use_undetected:
        try:
